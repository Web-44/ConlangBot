...
pub fn register() -> CreateCommand {
    CreateCommand::new("contributor")
        .description("Add/Remove users that can send messages in this channel")
        .add_option(CreateCommandOption::new(CommandOptionType::SubCommand, "add", "Add a contributor")
            .add_sub_option(CreateCommandOption::new(CommandOptionType::User, "user", "The user to add").required(true)))
        .add_option(CreateCommandOption::new(CommandOptionType::SubCommand, "remove", "Remove a contributor")
                ...
                if let Some(user) = options[0].value.as_user_id() {
                    ...
                    if let Ok(discord_channel) = cmd.channel_id.to_channel(&ctx).await {
                        ...
                        match cmd.data.options[0].name.as_str() {
                            "add" => {
                                let _ = discord_channel.create_permission(&ctx, user_collaborator!(user)).await;
                                let _ = cmd.edit_response(&ctx, EditInteractionResponse::new()
                                    .content(format!("Contributor added: <@{user}>"))
                            ...
                            "remove" => {
                                ...
                                let _ = cmd.edit_response(&ctx, EditInteractionResponse::new()
                                    .content(format!("Contributor removed: <@{user}>"))
                                    .allowed_mentions(CreateAllowedMentions::new())).await;
                            }
                            _ => {}
                        }
